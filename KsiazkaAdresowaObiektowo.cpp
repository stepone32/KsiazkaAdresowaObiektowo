...
#include <sstream>
#include <windows.h>
#include "KsiazkaAdresowaObiektowo.h"

using namespace std;

string loadLineOfText();

User::User(int id, string name, string passw)
{
    idOfUser=id;
    userName=name;
    password=passw;
...
int User::logging(vector<User> users)
{
    User userLog;
    string userName, password;
    cout << "Podaj nazwe uzytkownika: ";
...
void User::passwordChange(vector<User> &users, int idUser)
{
    string password;
    cout << "Podaj haslo:";
    password=loadLineOfText();
...
void User::enterTheUsersDetails(vector<User> &users)
{
    User dataOfUser;
    if(!users.empty())
...
void User::saveUsersToTxtFile(vector<User> &users)
{
    fstream fileUsers;
...
void User::readUsersFromTxtFile(vector<User> &users)
{
    User dataOfUser;
